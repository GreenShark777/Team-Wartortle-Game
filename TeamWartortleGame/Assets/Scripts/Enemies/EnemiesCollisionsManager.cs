...
public class EnemiesCollisionsManager : MonoBehaviour
{
    ...
    //riferimento al player per il calcolo della direzione di knockback
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (!eh.IsEnemyDefeated())
        {
            ...
            if (collision.CompareTag("Weapon")) {
                StartCoroutine(GotHit(collision.GetComponent<WeaponStats>())); 
            }
    ...
    private IEnumerator GotHit(WeaponStats ws)
    {
        ...
        Vector2 calculatedPush = -(player.position - transform.position).normalized * ws.GetPushForce();
